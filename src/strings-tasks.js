...
function isPalindrome(str) {
  const myString = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  const myStringReversed = myString.split('').reverse().join('');
  return myStringReversed === myString;
}
...
function findLongestWord(sentence) {
  ...
  function wichLengthLonger(w1, w2) {
    return w2.length - w1.length;
  }
  arrayWords.sort(wichLengthLonger);
  return arrayWords[0];
}
